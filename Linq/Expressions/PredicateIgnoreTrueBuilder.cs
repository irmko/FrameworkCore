...
namespace SkyNET.Framework.Common.Linq.Expressions {
    /// <summary>
    /// Предикат-билдер, игнорирующий предикаты вида x => true, и не добавляет их в итоговое выражение.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public sealed class NonEmptyPredicateBuilder<T> : IPredicateBuilder<T> {
        #region Nested types
        ...
        public IPredicateBuilder<T> Or(Expression<Func<T, bool>> expression) {
            Add(expression, Expression.OrElse);
            return this;
        }

        public IPredicateBuilder<T> OrNot(Expression<Func<T, bool>> expression) {
            var c = Expression.Lambda<Func<T, bool>>(Expression.Not(expression.Body), expression.Parameters);
            Add(c, Expression.OrElse);
            return this;
        }

        public IPredicateBuilder<T> And(Expression<Func<T, bool>> expression) {
            Add(expression, Expression.AndAlso);
            return this;
        }

        public IPredicateBuilder<T> AndNot(Expression<Func<T, bool>> expression) {
            var c = Expression.Lambda<Func<T, bool>>(Expression.Not(expression.Body), expression.Parameters);
        ...
        private void Add(Expression<Func<T, bool>> expression, Func<Expression, Expression, Expression> aggregate) {
            if (ExpressionBuilder.IsTrue(expression))
                return;
